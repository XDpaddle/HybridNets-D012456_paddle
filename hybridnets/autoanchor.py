...
def kmean_anchors(path='./data/coco128.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    ...
    def metric(k, wh):  # compute metrics
        r = wh[:, None] / k[None]
        x = paddle.min(r, 1. / r).min(2)[0]  # ratio metric
        # x = wh_iou(wh, torch.tensor(k))  # iou metric
        return x, x.max(1)[0]  # x, best_x

    def anchor_fitness(k):  # mutation fitness
        _, best = metric(paddle.to_tensor(k, dtype=paddle.float32), wh)
        return (best * paddle.cast((best > thr), 'float32')).mean()  # fitness

    def print_results(k):
        k = k[np.argsort(k.prod(1))]  # sort small to large
        x, best = metric(k, wh0)
        
        bpr, aat = (paddle.cast((best > thr), 'float32')).mean(), (paddle.cast((x > thr), 'float32')).mean() * n  # best possible recall, anch > thr
